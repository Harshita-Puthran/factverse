...
export function ValidateFacts() {
  ...
  const getScoreText = (score) => {
    if (score >= 0.7) return 'Highly Check-worthy';
    if (score >= 0.5) return 'Check-worthy Claim';
    if (score >= 0.3) return 'Possibly Factual';
    return 'Not a Factual Claim';
  };
        ...
        <CardContent>
          <form onSubmit={handleValidate} className="space-y-4">
            ...
            <Button 
              ...
              className="bg-gradient-to-r from-violet-500 to-purple-500 w-full sm:w-auto"
            >
              {isLoading ? (
                <>
            ...
            {results.results && results.results.length > 0 ? (
              results.results.map((result, index) => (
                <div key={index} className="border-b pb-4 last:border-b-0">
                  <p className="text-gray-800 italic mb-2">"{result.text}"</p>
